...
class GreedyRangeTest(cst.DataclassMixin):
    @dataclasses.dataclass
    ...
    entries: t.List[Entry] = cst.csfield(cs.GreedyRange(cst.DataclassStruct(Entry)))
    cnt: int = cst.csfield(cs.Computed(lambda ctx: len(ctx.entries)))
